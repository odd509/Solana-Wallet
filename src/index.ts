...
async function main() {
	...
	program
		...
		.description(
			"Transfer SOL to another wallet using public key or wallet name"
		)
		.option("-y, --yes", "Automatically approve the transaction")
		.action((otherPublicKey, amount, options) => {
			transfer(otherPublicKey, amount, options.yes);
		});
